...
This is the first test pattern I used to test my VGA controller. I just tried to output a simple but stable gradient that filled the whole monitor to test the resolution, pixel clock and timing parameters were functioning correctly before adding any mandelbrot rendering logic. If the display was not stable and was flickering, or off centre that would be a good indicator that the timing parameters were not correct. And if nothing appeared then that would be a good indicator that the sync signals were not actually being used to display over VGA, so that would be a good indicator of error in the VGA controller logic. 
<img width="2880" height="2160" alt="image" src="https://github.com/user-attachments/assets/58ebb0a3-a529-4b91-8393-667de6c2d9ba" />
